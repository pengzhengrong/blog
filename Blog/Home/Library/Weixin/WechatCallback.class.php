...
Class WechatCallback {
	...
	public function createMenu($btn1, $btn2, $btn3) {
		$myMenu = new MyMenu($btn1, $btn2, $btn3);
		return $myMenu->createMenu();
	}
	...
	public function btnClick($eventKey) {
		$postStr = $GLOBALS["HTTP_RAW_POST_DATA"];
		if (!empty($postStr)){
			$postObj = simplexml_load_string($postStr, 'SimpleXMLElement', LIBXML_NOCDATA);
			$fromUsername = $postObj->FromUserName;
			...
			$time = time();
			$arr1 = array('%toUser%', '%FromUser%', '%Time%', '%EVENTKEY%');
			$arr2 = array($toUsername, $fromUsername, $time, $eventKey);
			$response = str_replace($arr1, $arr2, $str); 
			exit($response);
		}
	}*/
}
