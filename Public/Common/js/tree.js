...
function MzTreeView(Tname)
{
  ...
  this.icons    = {
    ...
    PM3       : 'P3.gif',  //＋━
    empty     : 'L5.gif',     //空白图
    root      : 'manager.gif',   //缺省的根节点图标
    folder    : 'manager.gif', //缺省的文件夹图标
    file      : 'member.gif',   //缺省的文件图标
    exit      : 'exit.gif'
  };
...
MzTreeView.prototype.setStyle = function()
{
  ...
  */
  var style = "<style>"+
  "DIV.MzTreeView DIV IMG{border: 0px solid #FFFFFF;margin:0 2px 0 0;}"+
  "DIV.MzTreeView DIV SPAN IMG{border: 0px solid #FFFFFF;margin:0 2px 0 0;}";
  if(this.wordLine)
  {
    style +="\
    DIV.MzTreeView DIV\
...
MzTreeView.prototype.nodeToHTML = function(node, AtEnd)
{
  ...
  HTML += "<IMG "+
    ...
    "nodeid='"+node.sourceIndex.split("_")[1]+"'"+
    "href='"+ url +"' "+
    "target='"+ target +"' "+
    "title='"+ node.hint +"' "+
