...
class RelationModel extends Model {
    ...
    public function __call($method,$args) {
        // my_log('RelationModel  _call');  // many_to_may  select not this way
    ...
    public function getRelationTableName($relation) {
        // my_log('RelationModel  getRelationTableName');
    ...
    protected function _after_select(&$result,$options) {
         // my_log('RelationModel  _after_select  rest='.$result , json_encode( $options ) );
        // rest=Array={"link":true,"table":"think_user","model":"UserRelation"}
        // 获取关联数据 并附加到结果中
        if(!empty($options['link']))
    ...
    protected function _after_insert($data,$options) {
        // my_log('RelationModel _after_insert');
        // 关联写入
        if(!empty($options['link']))
    ...
    protected function getRelations(&$resultSet,$name='') {
        // my_log('RelationModel  getRelations  ' ,  json_encode( $resultSet ));
        // $name = true;
        // $resultSet = think_user data
        // 获取记录集的主键列表
    ...
    protected function getRelation(&$result,$name='',$return=false) {
          // my_log('getRelation');
        // my_log( 'result' , json_encode($result) );
        //2 ,true
        if(!empty($this->_link)) {
                    ...
                    if(empty($name) || true === $name || $mappingName == $name || (is_array($name) && in_array($mappingName,$name))) {
                        ...
                        switch($mappingType) {
                            case self::HAS_ONE:
                                ...
                                // my_log('where',$mappingCondition);  //cat_id=8
                            ...
                            case self::BELONGS_TO:
                                ...
                                $fk   =  $result[$mappingFk];
                                // my_log( 'mappingfk' ,$mappingFk );
                                // my_log('fk',$fk);
                                $mappingCondition .= " AND {$model->getPk()}='{$fk}'";
                                // my_log('condition',$mappingCondition);
                                $relationData   =  $model->where($mappingCondition)->field($mappingFields)->find();
                                if (!empty($val['relation_deep'])){
                                    $model->getRelation($relationData,$val['relation_deep']);
                                }                                
